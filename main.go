...
func main() {
	...
	// Let's read the PORT, ADMIN_USERNAME and ADMIN_PASSWORD example variables:
	port := os.Getenv("PORT")
	if port == "" {
		port = "8080"
	}
	...
	fmt.Printf("A web server will be started at port %s\n\n", port)
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "This web server is listening at port %s\n\n", port)
